...
class MiT(nn.Module):
    ...
    def __init__(
        ...
        logger.info(f"[MiT] grad_checkpointing: {grad_checkpointing}")
    ...
    def forward(
        ...
        use_grad_checkpointing = (
            ...
            else checkpoint_blocks
        )
        seq = self._build_sequence(x, h, omega=omega, t_min=t_min, t_max=t_max, y=y)

        for block in self.shared_blocks:
            if use_grad_checkpointing and self.training:
                def _checkpointed_block(seq_, block_=block):
                    return block_(seq_, self.rope_freqs)

                seq = checkpoint(_checkpointed_block, seq, use_reentrant=False)
            else:
        ...
        for block in self.u_heads:
            if use_grad_checkpointing and self.training:
                def _checkpointed_block(u_seq_, block_=block):
                    return block_(u_seq_, self.rope_freqs)

                u_seq = checkpoint(_checkpointed_block, u_seq, use_reentrant=False)
            else:
        ...
        for block in self.v_heads:
            if use_grad_checkpointing and self.training:
                def _checkpointed_block(v_seq_, block_=block):
                    return block_(v_seq_, self.rope_freqs)

                v_seq = checkpoint(_checkpointed_block, v_seq, use_reentrant=False)
            else:

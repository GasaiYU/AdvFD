...
class DiffusersAutoencoderKL(nn.Module):
    ...
    def decode(self, z, decode_bsz: int | None = None):
        """Decode latents, optionally in differentiable batch chunks.

        Chunk outputs are concatenated in their original order, so callers
        still receive one full batch and autograd propagates through every
        chunk.  This limits the VAE decoder's per-operation activation peak
        without changing the optimization batch.
        """
        if decode_bsz is not None and decode_bsz < 0:
            raise ValueError("decode_bsz must be >= 0 or None")
        z = z.to(dtype=next(self.vae.parameters()).dtype)
        if decode_bsz and z.shape[0] > decode_bsz:
            checkpoint_chunks = (
            ...
            def decode_chunk(z_chunk):
                return self.vae.decode(z_chunk)["sample"]

            decoded = [
                checkpoint(decode_chunk, z_chunk, use_reentrant=False)
                if checkpoint_chunks else decode_chunk(z_chunk)
                for z_chunk in z.split(decode_bsz, dim=0)
            ]

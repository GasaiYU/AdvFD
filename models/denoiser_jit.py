...
class JiTDenoiser(nn.Module):
    ...
    def sample_images_with_grad(self, x, y, sampling_args=None):
        ...
        t_start = sampling_args.get("t_start", None)
        return_velocity = sampling_args.get("return_velocity", False)
        if t_start is not None:
            t_cur = t_start.to(device=device, dtype=x.dtype).view(bsz, 1, 1, 1)

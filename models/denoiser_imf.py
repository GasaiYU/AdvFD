...
class iMFDenoiser(nn.Module):
    ...
    def __init__(
        ...
        logger.info(f"[iMF Denoiser] grad_checkpointing: {grad_checkpointing}")
    ...
    def u_fn(self, x, t, h, omega, t_min, t_max, y, checkpoint_blocks=None):
        bz = x.shape[0]
        return self.net(
    ...
    def forward(self, x, y):
        ...
        def run_jvp(z_in, t_in, r_in, v_c_in):
            def u_fn_for_dudt(z_arg, t_arg, r_arg):
                # torch.utils.checkpoint's custom autograd.Function cannot be
                # nested inside forward-mode AD. Checkpoint the complete JVP
                # below instead of checkpointing individual MiT blocks here.
                return self.u_fn(
                    z_arg,
                    t_arg,
                    t_arg - r_arg,
            ...
            return torch.func.jvp(
                u_fn_for_dudt,
                (z_in, t_in, r_in),
                (v_c_in, torch.ones_like(t_in), torch.zeros_like(r_in)),
                has_aux=True,
            )

        if self.net.grad_checkpointing and self.training:
            u, du_dt, v = checkpoint(
                run_jvp,
                z_t,
                t,
                r,

...
from frechet_distance.repr_models import inception_feature_layer_from_name, safe_model_name
...
def infer_stats_path(name, img_size, target_size, default_inception_path=None):
    ...
    if inception_feature_layer_from_name(name) is not None:
        target_size = 299
    sanitized = safe_model_name(name)
...
def extract_judge_features(judge, images):
    ...
    features.
    """
    inception_layer = judge.get("inception_layer")
    if inception_layer is not None and hasattr(judge["model"], "forward_features_by_layer"):
        primary, secondary = judge["model"].forward_features_by_layer(
            images,
            [inception_layer],
        )[inception_layer]
    else:
...
def extract_judge_features_all(judges, images):
    """Extract features for judges, sharing one Inception forward when possible."""
    features = [None] * len(judges)
    inception_groups = {}

    for idx, judge in enumerate(judges):
        inception_layer = judge.get("inception_layer")
        if inception_layer is None or not hasattr(judge["model"], "forward_features_by_layer"):
            features[idx] = extract_judge_features(judge, images)
            continue

        key = id(judge["model"])
        if key not in inception_groups:
            inception_groups[key] = {
                "model": judge["model"],
                "entries": [],
            }
        inception_groups[key]["entries"].append((idx, judge, inception_layer))

    for group in inception_groups.values():
        layers = [entry[2] for entry in group["entries"]]
        outputs = group["model"].forward_features_by_layer(images, layers)
        for idx, judge, inception_layer in group["entries"]:
            primary, secondary = outputs[inception_layer]
            features[idx] = secondary if judge.get("pool_type") == "avg" else primary
...
def fill_all_queues(judges, model, args, tokenizer=None):
    ...
    while filled < queue_size:
        ...
        local_feats_list = extract_judge_features_all(judges, imgs)

...
from utils.fsdp_util import is_fsdp_enabled
...
def _fsdp_state_dict_context(args, model, rank0_only=True):
    if not is_fsdp_enabled(args):
    ...
    return FSDP.state_dict_type(
        ...
        FullOptimStateDictConfig(offload_to_cpu=True, rank0_only=rank0_only),
    )
...
def save_checkpoint(
    ...
    """
    with _fsdp_state_dict_context(args, model):
        model_sd = model.state_dict()
        if is_fsdp_enabled(args):
            from torch.distributed.fsdp import FullyShardedDataParallel as FSDP

            optimizer_sd = FSDP.optim_state_dict(model, optimizer)
        else:

...
: "${FD_MAIN_REPRS:=inception}"  # inception | sim
: "${FD_ADV_REPRS:=follow}"      # follow | inception | sim
...
: "${FD_ADV_WEIGHT:=0.1}" # Need Fixing
: "${FD_ADV_LR:=2e-6}" # Need fixing 2e-6
: "${FD_ADV_STEPS:=1}"
: "${FD_ADV_GRAD_CLIP:=1.0}"
: "${FD_WHITEN:=1}"
: "${FD_WHITEN_EPS:=1e-3}"
: "${FD_ADV_START_STEP:=1000}" # Need Fixing
: "${FD_ADV_WARMUP_STEPS:=4000}" 
: "${FD_ADV_WHITEN_EPS:=1e-3}" # Need fixing
: "${FD_ADV_WHITEN:=1}"
: "${FD_ADV_NEG_REAL_DEGRADE_RATIO:=0}" # Need Fixing
: "${FD_ADV_LOG_RAW:=1}"
...
run_one "${MODEL}-${FD_MAIN_TAG}${FD_ADV_TAG}-advfd-repr-w${FD_ADV_WEIGHT}-gclip${FD_ADV_GRAD_CLIP}-negreal${FD_ADV_NEG_REAL_DEGRADE_RATIO}${FD_WHITEN_SUFFIX}${FD_ADV_WHITEN_SUFFIX}-adv${FD_ADV_START_STEP}-from-${LOAD_INIT}" \
    "${FD_REPR_ARGS[@]}" \
    "${FD_ADV_REPR_ARGS[@]}" \

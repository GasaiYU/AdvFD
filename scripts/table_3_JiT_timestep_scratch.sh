...
run_one() {
    ...
    torchrun \
        ...
        --fd_timestep_min_start "$FD_T_MIN_START" \
        --fd_timestep_min_end "$FD_T_MIN_END" \
        --fd_timestep_anneal_steps "$FD_T_ANNEAL_STEPS" \
        "$WANDB_FLAG" \
        "$@"
}

run_one "${MODEL}-fd-inception-timestep-scratch" --fd_repr_models inception
# run_one "${MODEL}-fd-sim-timestep-scratch" \

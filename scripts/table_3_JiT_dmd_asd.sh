#!/usr/bin/env bash
# Table 3 JiT FD + original DMD with ASD fake-score loss at 256px.
# Set MODEL_SIZE in {B,L,H}. Tune the ASD term with DMD_ASD_GAMMA.
...
EXP_NAME="${MODEL}-fd-inception-dmd-asd-w${DMD_GUIDANCE_WEIGHT}-r${DMD_GENERATOR_UPDATE_RATIO}-g${DMD_ASD_GAMMA}${DMD_DISCRIMINATOR_ONLY_SUFFIX}"
...
torchrun \
    ...
    --dmd_fake_loss_type asd \
    --dmd_fake_prediction_type "$DMD_FAKE_PREDICTION_TYPE" \
    --dmd_asd_gamma "$DMD_ASD_GAMMA" \
    "${DMD_DISCRIMINATOR_ONLY_ARGS[@]}" \

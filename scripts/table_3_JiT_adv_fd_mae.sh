#!/usr/bin/env bash
# Table 3: JiT scalability at 256px with adversarial FD using MAE repr.
...
run_one "${MODEL}-mae-adv-lora-r${FD_ADV_LORA_RANK}-rf${FD_ADV_REAL_UPDATE_FREQ}-from-${LOAD_INIT}" \
    --fd_repr_models "$MAE" \

#!/usr/bin/env bash
# Table 3: JiT scalability at 256px with SIM main FD and SigLIP LoRA FD-Adv.
...
: "${FD_ADV_LORA_RANK:=8}"
: "${FD_ADV_LORA_ALPHA:=16}"
: "${FD_ADV_LORA_TARGETS:=attn.qkv}"
: "${FD_ADV_LORA_DROPOUT:=0.0}"
: "${FD_ADV_REPR_GRAD_CHECKPOINTING:=1}"
...
run_one "${MODEL}-fd-sim-adv-siglip-lora-r${FD_ADV_LORA_RANK}${FD_ADV_REPR_GRAD_CHECKPOINT_SUFFIX}${FD_ADV_DETACH_REAL_SUFFIX}${FD_ADV_UPDATE_SUFFIX}${FD_ADV_REAL_UPDATE_SUFFIX}${FD_WHITEN_SUFFIX}${FD_ADV_WHITEN_SUFFIX}-w${FD_ADV_WEIGHT}-from-${LOAD_INIT}" \
    --fd_repr_models "$SIGLIP" "$MAE" inception \
    --fd_repr_pool_types cls cls cls \
    --fd_target_sizes 224 224 256 \
    --fd_adv_repr_models "$SIGLIP" \
    --fd_adv_repr_pool_types cls \
    --fd_adv_target_sizes 224 \

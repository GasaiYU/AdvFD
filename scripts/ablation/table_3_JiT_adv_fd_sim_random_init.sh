#!/usr/bin/env bash
# Table 3 ablation: SIM main FD with a scratch-initialized Inception FD-Adv.
...
: "${FD_ADV_FREEZE_BATCHNORM:=0}"  # Keep the same trainable BN affine policy as the pretrained control.
: "${FD_ADV_RANDOM_INIT_STATS_IMAGES:=50000}"
: "${FD_WHITEN:=0}"
: "${FD_WHITEN_EPS:=1e-3}"
...
FD_ADV_FREEZE_BATCHNORM_ARGS=()
if [ "$FD_ADV_FREEZE_BATCHNORM" = "1" ]; then
    FD_ADV_FREEZE_BATCHNORM_ARGS=(--fd_adv_freeze_batchnorm)
fi
...
run_one "${MODEL}-fd-sim-advinc-randominit-w${FD_ADV_WEIGHT}-from-${LOAD_INIT}" \
    ...
    --fd_adv_inception_random_init \
    --fd_adv_random_init_stats_images "$FD_ADV_RANDOM_INIT_STATS_IMAGES" \
    --fd_adv_lr "$FD_ADV_LR" \

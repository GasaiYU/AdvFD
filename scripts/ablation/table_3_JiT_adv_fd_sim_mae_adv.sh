#!/usr/bin/env bash
# Table 3: JiT scalability at 256px with SIM main FD and MAE LoRA FD-Adv.
...
run_one "${MODEL}-fd-sim-adv-mae-lora-r${FD_ADV_LORA_RANK}${FD_ADV_REPR_GRAD_CHECKPOINT_SUFFIX}${FD_ADV_DETACH_REAL_SUFFIX}${FD_ADV_UPDATE_SUFFIX}${FD_ADV_REAL_UPDATE_SUFFIX}${FD_WHITEN_SUFFIX}${FD_ADV_WHITEN_SUFFIX}-w${FD_ADV_WEIGHT}-from-${LOAD_INIT}-tmp" \
    --fd_repr_models "$SIGLIP" "$MAE" inception \
    --fd_repr_pool_types cls cls cls \
    --fd_target_sizes 224 224 256 \
    --fd_adv_repr_models "$MAE" \

#!/usr/bin/env bash
# Table 3: JiT scalability at 256px with SIM main FD and Inception FD-Adv.
...
: "${FD_ADV_LOG_FEATURE_SCALE_FREQ:=20}"  # Match --print_freq so every scale sample reaches W&B.
...
FD_ADV_LOG_FEATURE_SCALE_ARGS=()
if [ "$FD_ADV_LOG_FEATURE_SCALE" = "1" ]; then
    FD_ADV_LOG_FEATURE_SCALE_ARGS=(
        --fd_adv_log_feature_scale
...
run_one "${MODEL}-fd-sim-advinc-w${FD_ADV_WEIGHT}-from-${LOAD_INIT}-no-whiten" \

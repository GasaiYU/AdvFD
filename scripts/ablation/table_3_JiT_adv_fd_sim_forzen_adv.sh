#!/usr/bin/env bash
# Table 3 ablation: SIM main FD with frozen pretrained Inception FD-Adv.
# There is no D-step in this run; Inception stays fixed throughout.
...
: "${FD_ADV_LR:=0.0}"
: "${FD_ADV_STEPS:=0}"
: "${FD_ADV_UPDATE_FREQ:=1}"
: "${FD_ADV_GRAD_CLIP:=0.0}"
...
if [ "$FD_ADV_STEPS" != "0" ]; then
    echo "[ERR] frozen-adv ablation requires FD_ADV_STEPS=0 (no D-step)"
    exit 1
fi
FD_WHITEN_ARGS=()
FD_WHITEN_SUFFIX=
...
run_one "${MODEL}-fd-sim-frozenadvinc-w${FD_ADV_WEIGHT}-from-${LOAD_INIT}" \

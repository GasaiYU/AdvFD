...
from frechet_distance.metrics import (
    compute_precision_recall, compute_mmd, compute_mind, compute_mind_curve,
)
...
def _reduce_and_compute(repr_models, accumulators, inception_logits,
                        ...
                        mind_ref_features=None, mind_model_names=None,
                        mind_num_projections=1000, mind_sample_size=5000,
                        mind_sample_sizes=None,
                        mind_rng_seed=2021, mind_projection_batch_size=1000,
                        prc_k=3, prc_batch_size=5000):
    """Reduce across ranks, compute FD / IS / P&R / CMMD / MIND.

    ``local_feat_lists`` collects per-sample features for the union of
    P&R, CMMD, and MIND models. Each metric uses its own reference features.
    """
    ...
    mind_ref_features = mind_ref_features or {}
    mind_model_names = set(mind_model_names or [])
    mind_sizes = sorted(set(mind_sample_sizes or [mind_sample_size]))
    feat_model_names = prc_model_names | mmd_model_names | mind_model_names
    ...
    for feat_name in sorted(feat_model_names):
        ...
        do_mind = feat_name in mind_model_names
        ...
        if rank == 0:
            ...
            if do_mind:
                mind_ref = mind_ref_features[feat_name].to(device=device, dtype=torch.float32)
                if len(mind_sizes) == 1:
                    sample_size = mind_sizes[0]
                    mind_n = min(sample_size, mind_ref.shape[0], gen_gpu.shape[0])
                    logger.info(f"MIND ({feat_name}): ref={mind_ref.shape[0]}, "
                                f"gen={gen_gpu.shape[0]}, n={mind_n}, "
                                f"M={mind_num_projections}")
                    mind_val = {
                        sample_size: compute_mind(
                            mind_ref, gen_gpu,
                            num_projections=mind_num_projections,
                            max_samples=sample_size,
                            rng_seed=mind_rng_seed,
                            projection_batch_size=mind_projection_batch_size,
                        )
                    }
                else:
                    logger.info(f"MIND curve ({feat_name}): ref={mind_ref.shape[0]}, "
                                f"gen={gen_gpu.shape[0]}, sizes={mind_sizes}, "
                                f"M={mind_num_projections}")
                    mind_val = compute_mind_curve(
                        mind_ref, gen_gpu,
                        mind_sizes,
                        num_projections=mind_num_projections,
                        rng_seed=mind_rng_seed,
                ...
                for sample_size, val in sorted(mind_val.items()):
                    mind_n = min(sample_size, mind_ref.shape[0], gen_gpu.shape[0])
                    metrics_str.append(f"MIND@{mind_n}={val:.4f}")
                del mind_ref
            else:
                mind_val = None
            ...
            prec, rec, mmd_val, mind_val = None, None, None, None
            del all_gen
        ...
        if do_mind:
            mind_results[feat_name] = {}
            for sample_size in mind_sizes:
                v = mind_val.get(sample_size, 0.0) if mind_val is not None else 0.0
                mind_results[feat_name][sample_size] = broadcast_scalar(v)
        torch.cuda.empty_cache()

    return results, is_val, prc_results, mmd_results, mind_results
...
def _generate_and_evaluate(args, model, ema_model, repr_models, cfg, ema_label,
                           ...
                           mmd_ref_features=None, mmd_model_names=None,
                           mind_ref_features=None, mind_model_names=None):
    """Generate images and compute FD/IS/P&R/CMMD/MIND."""
    ...
    mind_model_names = mind_model_names or []
    feat_model_names = sorted(set(prc_model_names) | set(mmd_model_names) | set(mind_model_names))
    ...
    results, is_val, prc_results, mmd_results, mind_results = _reduce_and_compute(
        ...
        mind_ref_features=mind_ref_features, mind_model_names=mind_model_names,
        mind_num_projections=args.mind_num_projections,
        mind_sample_size=args.mind_sample_size,
        mind_sample_sizes=args.mind_sample_sizes,
        mind_rng_seed=args.mind_rng_seed,
        mind_projection_batch_size=args.mind_projection_batch_size,
    ...
    return results, is_val, gen_time, prc_results, mmd_results, mind_results
...
def _evaluate_from_folder(repr_models, image_dir=None, *,
                          ...
                          mind_rng_seed=2021, mind_projection_batch_size=1000,
                          prc_k=3, prc_batch_size=5000):
    """Load images from a folder (or pre-built dataset) and compute FD/IS/P&R/CMMD/MIND.
    ...
    results, is_val, prc_results, mmd_results, mind_results = _reduce_and_compute(
        ...
        mind_sample_sizes=mind_sample_sizes,
    ...
    return results, is_val, elapsed, total_count, prc_results, mmd_results, mind_results
...
def _load_repr_models(model_names, img_size, target_size=TARGET_SIZE,
    ...
    for name in model_names:
        ...
        if is_internvl_model(name) and name not in target_size_overrides:
            ts_override = 224
...
def _load_prc_refs(prc_model_names, repr_models, prc_ref_dir, *,
                   ref_label="P&R", max_images=None, rng_seed=2021):
    """Load reference features using all GPUs.
    ...
    for prc_raw in prc_model_names:
        ...
        if model_id not in extracted:
            if not os.path.isdir(prc_ref_dir):
                logger.warning(f"No {ref_label} ref dir for {short}; skipping")
                continue
            ...
            cache_extra = "" if max_images is None else f"_n{max_images}_seed{rng_seed}"
            cache_paths = {
                pool: f"{prc_ref_dir.rstrip('/')}_{suffix}{cache_extra}.pt"
            ...
            if all_cached and rank == 0:
                ...
                for pool, cp in cache_paths.items():
                    ...
                    logger.info(f"Loaded cached {ref_label} ref from {cp} ({result[pool].shape})")
                ...
                def both_fn(x, m=repr_model, logits=has_logits, dual=has_dual):
                    if logits:
                        return m(x)[0]
                    use_amp = not getattr(m, "is_inception", False)
                    with torch.autocast("cuda", enabled=use_amp, dtype=torch.bfloat16):
                        out0, out1 = m(x)
                ...
                all_feats = extract_ref_features(
                    ...
                    max_images=max_images, rng_seed=rng_seed)
                    ...
                    for _, suffix, pool in entries_to_register:
                        ...
                        if f is not None:
                            ...
                            try:
                                ...
                                torch.save(f, cp)
                                logger.info(f"Cached {ref_label} ref ({suffix}): {f.shape} -> {cp}")
                            except OSError as exc:
                                logger.warning(f"Could not cache {ref_label} ref to {cp}: {exc}")
                else:
...
CSV_FIELDS = [
    "model", "fd", "fdr", "fdr6", "is", "precision", "recall", "cmmd",
    "mind", "mind_n", "mind_m", "n",
...
def _resolve_prc(results, is_val, prc_results, mmd_results=None, mind_results=None):
    """Resolve per-row derived metrics (inception rows share IS/P&R/CMMD)."""
    mmd_results = mmd_results or {}
    mind_results = mind_results or {}
    inception_labels = {label for label, _ in INCEPTION_STATS}
    ...
    inception_mind = next((v for name, v in mind_results.items()
                           if name in inception_labels), None)
    ...
    for rname, fd_val in results.items():
        ...
        mind_val = inception_mind if is_inception else mind_results.get(rname, None)
        fdr = fd_val / FDR_VALIDATION_FD[rname] if rname in FDR_VALIDATION_FD else None
        if isinstance(mind_val, dict):
            mind_items = sorted(mind_val.items())
        else:
            mind_items = [(None, mind_val)]
        for mind_n, mind_score in mind_items:
            rows.append({
                "model": rname, "fd": fd_val, "fdr": fdr, "fdr6": fdr6,
                "is": is_val if is_inception else None,
                "precision": prec, "recall": rec, "cmmd": mmd_val,
                "mind": mind_score, "mind_n": mind_n,
            })
    return rows


def _build_csv_rows(results, is_val, prc_results, num_images, context,
                    mmd_results=None, mind_results=None):
    """Build wide-format CSV rows (one row per model) for _append_csv."""
    def _round(v, n):
        return round(v, n) if v is not None else None

    rows = []
    for e in _resolve_prc(results, is_val, prc_results, mmd_results, mind_results):
        row_context = dict(context)
        fallback_mind_n = row_context.pop("mind_n", None)
        mind_n = e.get("mind_n")
        if mind_n is None:
            mind_n = fallback_mind_n
        if e["mind"] is None:
            mind_n = None
        mind_m = row_context.pop("mind_m", None) if e["mind"] is not None else None
        rows.append({
            ...
            "mind": _round(e["mind"], 6), "mind_n": mind_n, "mind_m": mind_m,
            **row_context,
        })
    return rows
...
def _log_summary_table(results, is_val, prc_results, num_images, title,
    ...
    entries = _resolve_prc(results, is_val, prc_results, mmd_results, mind_results)
    has_prc = any(e["precision"] is not None for e in entries)
    has_mmd = any(e["cmmd"] is not None for e in entries)
    has_mind = any(e["mind"] is not None for e in entries)
    ...
    for e in entries:
        ...
        if has_mind:
            vals.append(f"{e['mind']:14.4f}" if e["mind"] is not None else f"{'N/A':>14}")
...
def _print_csv_summary(csv_path):
    ...
    for col in ("fd", "fdr", "fdr6", "mind", "mind_n", "mind_m", "n", "cfg", "step"):
        if col in df:
    ...
    def _fmt_count(v):
        if pd.isna(v):
            return "?"
        v = int(v)
        return f"{v // 1000}k" if v % 1000 == 0 and v >= 1000 else str(v)

    if "n" not in df:
        df["n"] = -1

    for n_val in sorted(v for v in df["n"].dropna().unique()):
        sub_df = df[df["n"] == n_val].copy()
        n_label = _fmt_count(n_val)

        logger.info("\n" + "=" * 70)
        logger.info(f"BEST FD PER MODEL (N={n_label})")
        logger.info("=" * 70)
        for model_name in sub_df["model"].unique():
            sub = sub_df[sub_df["model"] == model_name].dropna(subset=["fd"])
            if sub.empty:
                continue
            ...
            mind_str = ""
            if pd.notna(best.get("mind")):
                mind_label = _fmt_count(best.get("mind_n"))
                mind_str = f"  MIND@{mind_label}={best['mind']:.4f}"
            logger.info(f"  {model_name:>20s}: FD={best['fd']:.4f}{fdr_str}{fdr6_str}"
                         f"{mind_str}{prec_str}  cfg={best['cfg']}  ema={best['ema_label']}")

        logger.info("\n" + "=" * 70)
        logger.info(f"FULL RESULTS (N={n_label})")
        logger.info("=" * 70)
        fd_pivot = sub_df.pivot_table(
            index=["ema_label", "cfg"], columns="model",
            values="fd", aggfunc="last",
        )
        display = fd_pivot.copy()
        if "mind" in sub_df and sub_df["mind"].notna().any():
            mind_pivot = sub_df.pivot_table(
                index=["ema_label", "cfg"], columns="model",
                values="mind", aggfunc="last",
            )
            mind_n_vals = sub_df.loc[sub_df["mind"].notna(), "mind_n"].dropna().unique()
            mind_m_vals = sub_df.loc[sub_df["mind"].notna(), "mind_m"].dropna().unique()
            mind_n_label = _fmt_count(mind_n_vals[0]) if len(mind_n_vals) == 1 else "?"
            mind_m_label = _fmt_count(mind_m_vals[0]) if len(mind_m_vals) == 1 else "?"
            suffix = f" MIND@{mind_n_label}/M{mind_m_label}"
            for col in mind_pivot.columns:
                display[f"{col}{suffix}"] = mind_pivot[col]
        logger.info("\n" + display.to_string())
...
def main_folder(args):
    ...
    eval_mind = getattr(args, "eval_mind", False)
    prc_model_list = getattr(args, "prc_models", None) or args.models
    mind_model_list = getattr(args, "mind_models", None) or args.models
    ...
    if eval_mmd:
        mmd_ref_features, mmd_names = _load_prc_refs(
            prc_model_list, repr_models, args.mmd_ref_dir, ref_label="CMMD")

    mind_ref_features, mind_names = {}, []
    if eval_mind:
        mind_ref_max_images = max(args.mind_sample_sizes or [args.mind_sample_size])
        mind_ref_features, mind_names = _load_prc_refs(
            mind_model_list, repr_models, args.mind_ref_dir,
            ref_label="MIND", max_images=mind_ref_max_images,
            rng_seed=args.mind_rng_seed)
    ...
    for folder, out_csv in zip(folders, csv_list):
        ...
        results, is_val, elapsed, num_images, prc_results, mmd_results, mind_results = _evaluate_from_folder(
            repr_models, folder,
            ...
            mind_ref_features=mind_ref_features, mind_model_names=mind_names,
            mind_num_projections=args.mind_num_projections,
            mind_sample_size=args.mind_sample_size,
            mind_sample_sizes=args.mind_sample_sizes,
            mind_rng_seed=args.mind_rng_seed,
        ...
        if rank == 0 and results:
            _log_summary_table(results, is_val, prc_results, num_images,
                               f"Summary: {folder}", mmd_results, mind_results)
            if out_csv:
                ...
                if eval_mind:
                    context["mind_n"] = args.mind_sample_size
                    context["mind_m"] = args.mind_num_projections
                _append_csv(out_csv,
                            _build_csv_rows(results, is_val, prc_results, num_images,
                                            context, mmd_results, mind_results))
...
def main_random_train(args):
    ...
    if eval_mmd:
        mmd_ref_features, mmd_names = _load_prc_refs(
            args.models, repr_models, args.mmd_ref_dir, ref_label="CMMD")

    mind_ref_features, mind_names = {}, []
    if eval_mind:
        mind_models = getattr(args, "mind_models", None) or args.models
        mind_ref_max_images = max(args.mind_sample_sizes or [args.mind_sample_size])
        mind_ref_features, mind_names = _load_prc_refs(
            mind_models, repr_models, args.mind_ref_dir,
            ref_label="MIND", max_images=mind_ref_max_images,
            rng_seed=args.mind_rng_seed)
    ...
    for seed in seeds:
        ...
        if rank == 0 and results:
            _log_summary_table(results, is_val, prc_results, num_images,
                               f"Summary: train subset seed={seed}", mmd_results, mind_results)
            if base_csv:
                ...
                _append_csv(csv_path,
                            _build_csv_rows(results, is_val, prc_results,
                                            num_images, context, mmd_results, mind_results))
...
def main_generate(args):
    ...
    mind_models = getattr(args, "mind_models", None) or list(args.models)
    ...
    if eval_mmd:
        mmd_ref_features, mmd_names = _load_prc_refs(
            prc_models, repr_models, args.mmd_ref_dir, ref_label="CMMD")
    ...
    if eval_mind:
        ...
        mind_ref_features, mind_names = _load_prc_refs(
            ...
            rng_seed=args.mind_rng_seed)
    inception_labels = {label for label, _ in INCEPTION_STATS}
    mind_required_names = set(mind_names)
    if mind_required_names & inception_labels:
        mind_required_names |= inception_labels
    requested_mind_sizes = set(args.mind_sample_sizes or [args.mind_sample_size])
    ...
    existing = set()  # (ema_label, cfg, model, step, n)
    if rank == 0 and os.path.exists(csv_path):
        import pandas as _pd
        _df = _pd.read_csv(csv_path, on_bad_lines="skip")
        if eval_mind:
            group_cols = ["ema_label", "cfg", "model", "step", "n"]
            for key, group in _df.groupby(group_cols, dropna=False):
                ema_label, cfg, model_name, step, n_val = key
                if str(model_name) in mind_required_names:
                    if not all(c in group.columns for c in ("mind", "mind_n", "mind_m")):
                        have_sizes = set()
                    else:
                        valid_mind = group[
                            group["mind"].notna()
                            & group["mind_n"].notna()
                            & group["mind_m"].notna()
                            & (group["mind_m"].astype(int) == int(args.mind_num_projections))
                        ]
                        have_sizes = set(valid_mind["mind_n"].astype(int).tolist())
                    if not requested_mind_sizes.issubset(have_sizes):
                        continue
                existing.add((str(ema_label), float(cfg), str(model_name), int(step), int(n_val)))
        else:
            for _, row in _df.iterrows():
                n_val = int(row["n"]) if "n" in row and _pd.notna(row.get("n")) else -1
                existing.add((
                    str(row["ema_label"]), float(row["cfg"]), str(row["model"]),
                    int(row["step"]), n_val,
                ))
    ...
    for ema_label in ema_labels:
        for cfg_val in cfg_list:
            ...
            if n_existing > 0:
                ...
                cached_flags = [
                    broadcast_scalar(float(
                        rank == 0
                        and (ema_label, cfg_val, rn, args.current_step, args.num_images) in existing
                    ))
            ...
            results, is_val, gen_time, prc_results, mmd_results, mind_results = _generate_and_evaluate(
            ...
            csv_rows = _build_csv_rows(results, is_val, prc_results,
                                        args.num_images, context, mmd_results, mind_results)
            if rank == 0:
...
def _get_folder_parser():
    ...
    parser.add_argument("--eval_mind", action="store_true",
                        help="compute MIND for selected models")
    parser.add_argument("--mind_models", type=str, nargs="+", default=None,
                        help="models for MIND (default: same as --models)")
    parser.add_argument("--mind_ref_dir", type=str, default="./data/imagenet/val",
                        help="reference image dir for MIND")
    parser.add_argument("--mind_sample_size", type=int, default=5000,
                        help="number of real/generated features used for MIND")
    parser.add_argument("--mind_sample_sizes", type=int, nargs="+", default=None,
                        help="multiple sample sizes for a MIND curve")
    parser.add_argument("--mind_num_projections", type=int, default=1000,
                        help="number of random projection directions for MIND")
    parser.add_argument("--mind_projection_batch_size", type=int, default=1000,
                        help="projection chunk size for MIND computation")
    parser.add_argument("--mind_rng_seed", type=int, default=2021,
                        help="random seed for MIND ref sampling and projections")
